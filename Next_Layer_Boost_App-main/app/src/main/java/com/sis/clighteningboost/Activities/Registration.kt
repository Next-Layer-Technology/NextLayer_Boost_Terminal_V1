    ...
    private void initRPCResponse() {
        rpcResponse = new RPCResponse() {
            ...
            public void getRPCResponse(String response, int responseType) {
                switch (responseType) {
                    case Utils.CREATE_INVOICE_RESPONSE:
                        createInvoiceResponse(response);
                        break;
                    case Utils.CONFIRM_INVOICE_RESPONSE:
                        confirmInvoiceResponse(response);
                        break;
                    case Utils.CONNECT_TO_NETWORK:
                        connectToNetworkResponse(response);
                        break;
                    case Utils.CONNECT_TO_NETWORK_FOR_VALIDATE_USER:
                        validateUserResponse(response);
                        break;
                    case Utils.GET_TAX_INFO_FROM_SERVER_RESPONSE:
                        getTaxInfoFromServer(response);
                        break;
                    default:
                        break;
                }
...
//        RequestBody max_daily_boost = RequestBody.create(MediaType.parse("text/plain"),merchantData.getMerchant_maxboost());
    ...
    private void sendEmailFromClass(ClientData clientData) {
        ...
        if (clientData.getIs_active().equals("0")) {
    ...
    private void reConfirmRegistrationInvoice2(String query, String ip, String port, String username, String password) {
        ...
        String merchantPwd = sp.getStringValue("merchant_password");
        String url = ip + ":" + port;
